...
class MaskedLinear(nn.Linear):
    ...
    def __init__(
        ...
        if self.pruning_method in ["topK", "threshold", "sigmoied_threshold", "l0"]:
            ...
            self.scale = nn.Parameter(torch.empty((1,self.weight.size(0))))


    def forward(self, input: torch.tensor, threshold = 1):
        
        output = nn.functional.linear(input, self.weight)
        mask = TopKBinarizer.apply(self.scale, threshold)

        # Mask weights with computed mask
        output = output * (self.scale * mask)
...
class MaskedBertSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class MaskedBertSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class MaskedBertAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = MaskedBertSelfAttention(config)
        self.output = MaskedBertSelfOutput(config)
...
class MaskedBertIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = MaskedLinear(
            config.hidden_size,
...
class MaskedBertOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = MaskedLinear(
            config.intermediate_size,
...
class MaskedBertLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.attention = MaskedBertAttention(config)
        self.is_decoder = config.is_decoder
        if self.is_decoder:
            self.crossattention = MaskedBertAttention(config)
        self.intermediate = MaskedBertIntermediate(config)
...
class MaskedBertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.output_attentions = config.output_attentions
